...
func (dt *DateTime) MarshalText() ([]byte, error) {
	...
	return []byte(dt.UTC().Format(time.RFC1123)), nil
}

func (dt *DateTime) UnmarshalText(text []byte) (err error) {
	var input string
	if input = strings.TrimSpace(string(text)); len(input) == 0 {
		return fmt.Errorf("date string is empty")
	}
	var format string
	var zone *time.Location
	for _, format = range dateFormats {
		if dt.Time, err = time.Parse(format, input); err != nil {
			continue // try next format
		}
		if zone = dt.Location(); zone == nil {
			return // no time zone info, but success
		}
		if zone, err = time.LoadLocation(zone.String()); err != nil {
			return // failed to load location
		}
		if dt.Time, err = time.ParseInLocation(format, input, zone); err == nil {
			return // success
		}
	}
	return fmt.Errorf("failed to parse date: %q", input)
}
